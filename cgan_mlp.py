...
class CGAN():
	def __init__(self, generator, discriminator, data):
		...
		self.z_dim = self.data.z_dim  # z_dim=128
		self.y_dim = self.data.y_dim  # condition y_dim=10
		self.size = self.data.size  # size=28

		self.channel = self.data.channel  # channel=1

		self.X = tf.placeholder(tf.float32, shape=[None, self.X_dim])  # (?,784)
		self.z = tf.placeholder(tf.float32, shape=[None, self.z_dim])  # (?,128)
		self.y = tf.placeholder(tf.float32, shape=[None, self.y_dim])  # (?,10)

		# nets
		self.G_sample = self.generator(self.z, self.y)  # (?,138)

		self.D_real = self.discriminator(concat(self.X,self.y))
		self.D_fake = self.discriminator(concat(self.G_sample,self.y), reuse=True)
	...
	def train(self, sample_dir, ckpt_dir='ckpt', training_epoches = 1000000, batch_size = 16):
		...
		for epoch in range(training_epoches):
			...
			# save img, model. print loss
				if epoch % 100 == 0 :
					D_loss_curr, prob = self.sess.run(self.D_loss,
						feed_dict={self.X: X_b, self.y: y_b, self.z: sample_z(batch_size, self.z_dim)})
					G_loss_curr, w, shape_z = self.sess.run(self.G_loss,
						feed_dict={self.y: y_b, self.z: sample_z(batch_size, self.z_dim)})
