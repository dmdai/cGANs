...
class CGAN_Classifier(object):
	def __init__(self, generator, discriminator, classifier, data):
		...
		self.X = tf.placeholder(tf.float32, shape=[None, self.size, self.size, self.channel])#(?,28,28,1)
		self.z = tf.placeholder(tf.float32, shape=[None, self.z_dim])#(?,128)
		self.y = tf.placeholder(tf.float32, shape=[None, self.y_dim])#(?,10)

		# nets
		self.G_sample, self.shape_z= self.generator(self.z, self.y)#(?,138)
			...
			for epoch in range(training_epoches):
				...
				if epoch % 100 == 0 :
					D_loss_curr, C_real_loss_curr = self.sess.run(
							...
							feed_dict={self.X: X_b, self.y: y_b, self.z: sample_z(batch_size, self.z_dim)})
					G_loss_curr, C_fake_loss_curr, _ = self.sess.run(
							[self.G_loss, self.C_fake_loss, self.shape_z],
							feed_dict={self.y: y_b, self.z: sample_z(batch_size, self.z_dim)})
					...
					if epoch % 1000 == 0:
						y_s = sample_y(64, self.y_dim, fig_count%10)
						samples = self.sess.run(self.G_sample, feed_dict={self.y: y_s,self.input_h:np.zeros((batch_size,128)), self.z: sample_z(64, self.z_dim)})
