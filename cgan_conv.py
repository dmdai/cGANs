...
def conv_concat(x,y):
	...
	return tf.concat([x, y*tf.ones([bz, 28, 28, 10])], 3)
...
class CGAN():
	def __init__(self, generator, discriminator, data):
		...
		self.D_real = self.discriminator(conv_concat(self.X, self.y))
		self.D_fake = self.discriminator(conv_concat(self.G_sample, self.y), reuse = True)
